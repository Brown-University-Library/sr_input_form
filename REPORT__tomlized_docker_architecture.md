...
The README documents fresh setup, both `.env` locations, file precedence, starter-data prerequisites, rebuilds, restart after settings changes, guarded tests, and offline management commands. Detailed implementation information remains in this report. The two outer deployment callers already source the uv callee and select `staging`/`prod`; their shared Python declarations remain compatible and the callers were not replaced. No maintained repository CI, cron, or IDE command requiring another migration was found. Live server crontabs and colleagues' private IDE configurations were not accessed.
...
### README command review — September 7, 2026

The README now separates installation from optional checks/tests and explicitly keeps the development server running in the first terminal while Docker checks run in a second terminal from the same application directory. Readiness is identified by Django's server-start message instead of waiting for logs to become quiet. Control-C, restarting, settings reloads, the test runner's `yes` prompt, and recovery from `service "web" is not running` are explained. Local uv setup supplies missing working-directory commands and identifies both required SQLite database paths. Advanced implementation details were shortened, and outdated Django examples were corrected. At the user's request, the README contains no branch-selection instructions; it describes the intended merged workflow. No runtime code or dependency declarations changed during this review.

The walkthrough used disposable copies under `/private/tmp/sr-readme-review-20260907/`, with separate Compose names, loopback ports 38000/33306/38080, and no mounts of the working installation's settings or databases. A fresh `up --build` was attempted twice but did not complete: MySQL image metadata resolution timed out with `DeadlineExceeded: context deadline exceeded`. Runtime validation therefore used the previously verified ARM64 web image `716671ba06c8`, the cached companion database image, and cached Adminer, with the current checkout mounted and the actual Compose startup command retained. This review does not claim a successful fresh image build or physical Windows/Linux host testing.

The Docker walkthrough confirmed automatic creation of the four working directories and sample settings, the server-readiness message, HTTP 200 responses from `/info/`, `/login/`, `/version/`, and Adminer, and a successful Django check. All 64 guarded tests passed after interactive `yes` while the server remained running. `restart web` succeeded. Control-C stopped the stack; the next `exec web ... check` reproduced the user's exact service-not-running error. Starting the same stack again restored the readiness message, HTTP access, and the check. These observations match Docker's descriptions of [foreground Compose startup and stopping](https://docs.docker.com/reference/cli/docker/compose/up/) and [commands in running containers](https://docs.docker.com/reference/cli/docker/compose/exec/).

A separate local uv checkout passed `uv sync --locked --group local`, the Django check, all 64 guarded tests, and the five tests selected by `disa_app.tests.test_renamer`. Its development server served the three documented application pages and stopped with Control-C. Local validation used a copied starter Django database and a synthetic SQLAlchemy database made with the repository's fixture builder, rather than a team's complete local dataset. Both environments retained the existing `caches.W003` warning. Evidence includes `foreground.log` (build failure), `cached-foreground.log`, `cached-restart.log`, `recovery.log`, and `local-sync.log`; successful interactive test results were captured in the review's tool output. Redirected preliminary test attempts were correctly refused because the runner requires interactive stdin and stderr; the subsequent unredirected runs exercised the documented manual workflow.
