...
Use an explicitly versioned Python base image matching the agreed TOML patch, preferably also pinned to a verified image digest and operating-system variant. A build assertion should compare `sys.version_info` with the TOML declaration so later edits cannot leave the Dockerfile on a different patch. If `.python-version` is added for convenience, derive it from the same decision and check agreement; it must not become an independent authority. uv supports exact interpreter requests and disabling automatic Python downloads. [uv Python version documentation](https://docs.astral.sh/uv/concepts/python-versions/).
...
Before broad edits, attempt the selected Python 3.8 patch plus the exact locked local dependencies in a minimal Linux image. Capture interpreter/version output and the complete build or import failure. Exercise imports of Django, SQLAlchemy, PyMySQL, and dotenv before database startup. Test the team's relevant architectures, including Linux ARM64 and Linux AMD64 where used.

### 2. Convert the single web Dockerfile to uv

1. Select the verified Python patch/base image and copy the uv binary from an explicitly versioned official uv image, preferably pinned by digest. Check that this uv release supports Python 3.8 and the existing lock format. Do not install uv through pip or use a moving `latest` tag.
2. Keep `WORKDIR /sr_project_stuff/code`, unbuffered output, and disabled bytecode writes. Ensure the log, SQLite, and cache directories exist. Preserve Bash for the current startup logic and provide the Git executable needed by `/version/`.
3. Set `UV_PROJECT_ENVIRONMENT=/opt/venv` and put `/opt/venv/bin` first on `PATH`. `/opt/venv` is outside both existing source/outer-directory mount destinations, so host mounts cannot cover the environment installed in the image.
...
5. Install with the equivalent of `uv sync --locked --no-default-groups --group local --python /usr/local/bin/python3 --no-python-downloads`. Verify the explicit interpreter path in the chosen image. A conflicting interpreter should fail rather than trigger a hidden second Python installation.
...
At startup, run a locked local sync against the mounted TOML/lockfile with `--offline` and the same explicit interpreter selection used at build time. This checks that the mounted checkout still matches its lockfile and installed environment without downloading replacements. If changed dependencies cannot be satisfied from the built image, fail with an instruction to rebuild. Verify this behavior with the selected uv release.
...
| Old image/interpreter availability differs by platform | Verify the exact Python image on each supported architecture. If unavailable, try a digest-pinned Debian image plus a pinned uv release installing the same exact Python patch. Keep the managed interpreter outside source mounts. Prove availability and imports rather than assuming an older download or image exists. BIRKIN-FEEDBACK: note that `uv` can install its own version of python, so you _might_ not need a python-specifc image. |
...
1. **Dependency and interpreter agreement:** validate lock freshness; build the web image; compare the actual Python patch and every runtime distribution against TOML/lock. Verify absence of old shellvars/pip-tools dependencies in the application environment. Independently sync `local`, `staging`, and `prod` in appropriate disposable environments so one group's installed packages cannot hide another's missing dependency. Production's native build check may need its server OS prerequisites.
